...
/**
  ...
  @param {string} realmName - The name of the realm(not the realmID) - ex: master,
  @param {object} [options] - The option object is required
  @param {object} [options.groupId] - The group where the user should be added to.
  @param {object} [options.id] - The user id of the user that should be added to a group.
  @returns {Promise} A promise that resolves.
  @example
  keycloakAdminClient(settings)
    .then((client) => {
      client.users.update_groups(realmName, {
        groupId: groupId,
        id: id
      })
        .then(() => {
 ...
 */
function update_groups (client) {
  return function update_groups (realmName, options) {
    return new Promise((resolve, reject) => {
      const req = {
        url: `${client.baseUrl}/admin/realms/${realmName}/users/${options.id}/groups/${options.groupId}`,
        auth: {
          bearer: privates.get(client).accessToken
        },
        json: true,
