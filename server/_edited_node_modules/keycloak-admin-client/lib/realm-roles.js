...
module.exports = {
  find: find,
  create: create,
  findUsersWithRole: findUsersWithRole
};
...
/**
 * THIS FUNCTION IS ONLY IN THIS MODIFIED VERSION BY Jan Sonntag
  A function to get the list of users with a specified role
  @param {string} realmName - The name of the realm(not the realmID) - ex: master
  @param {object} [options] - The options object
  @param {string} [options.name] - use this options to get the desired rolenmae
  @returns {Promise} A promise that will resolve with an Array of user objects or just the 1 user object if userId is used
  @example
  keycloakAdminClient(settings)
    .then((client) => {
      client.realms.roles.findUsersWithRole(realmName)
        .then((userList) => {
        console.log(userList) // [{...},{...}, ...]
      })
    })
 */
function findUsersWithRole (client) {
  return function findUsersWithRole (realm, options) {
    return new Promise((resolve, reject) => {
      options = options || {};
      const req = {
        auth: {
        ...
        req.url = `${client.baseUrl}/admin/realms/${realm}/roles/${options.name}/users`;
